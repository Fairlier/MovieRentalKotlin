...
import com.example.movierentalkotlin.viewmodel.movie.MovieCatalogViewModel
import com.example.movierentalkotlin.viewmodel.SharedViewModel
import com.example.movierentalkotlin.viewmodelfactory.movie.MovieCatalogViewModelFactory
...
class MovieCatalogFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
                return when (menuItem.itemId) {
                    R.id.searchMovieFragment -> {
                        val action =
                            MovieCatalogFragmentDirections.actionMovieCatalogFragmentToSearchMovieFragment()
        ...
        viewModel.navigateToView.observe(viewLifecycleOwner) { id ->
            id?.let {
                val action =
                    MovieCatalogFragmentDirections.actionMovieCatalogFragmentToViewMovieFragment(id)
        ...
        viewModel.navigateToInsert.observe(viewLifecycleOwner) { navigate ->
            if (navigate) {
                val action =
                    MovieCatalogFragmentDirections.actionMovieCatalogFragmentToInsertMovieFragment()
                this.findNavController().navigate(action)
