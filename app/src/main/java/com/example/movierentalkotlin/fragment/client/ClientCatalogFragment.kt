...
import com.example.movierentalkotlin.viewmodel.client.ClientCatalogViewModel
import com.example.movierentalkotlin.viewmodel.SharedViewModel
import com.example.movierentalkotlin.viewmodelfactory.client.ClientCatalogViewModelFactory
...
class ClientCatalogFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
            ...
            override fun onMenuItemSelected(menuItem: MenuItem): Boolean {
                return when (menuItem.itemId) {
                    R.id.searchClientFragment -> {
                        val action =
                            ClientCatalogFragmentDirections.actionClientCatalogFragmentToSearchClientFragment()
        ...
        viewModel.navigateToView.observe(viewLifecycleOwner) { id ->
            id?.let {
                val action =
                    ClientCatalogFragmentDirections.actionClientCatalogFragmentToViewClientFragment(
                        id
                    )
        ...
        viewModel.navigateToInsert.observe(viewLifecycleOwner) { navigate ->
            if (navigate) {
                val action =
                    ClientCatalogFragmentDirections.actionClientCatalogFragmentToInsertClientFragment()
                this.findNavController().navigate(action)
